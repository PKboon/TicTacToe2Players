...
#set window's title
pygame.display.set_caption('COMP-2100-09 Final Project Group 1: Tic-tac-toe Server')
#-----------------------------------------------------------------------------
#colors
BLACK  = (0, 0, 0)
WHITE  = (255, 255, 255)
...
#accepts connection and alerts when they are connected
def waitingForConnection():
    global connectionEstablished, connection, address
...
def receiveData():
    global turn, playing, imgO, score2, numOfImg
...
def gamePage():
    global score1, score2, gameOption, screen, welText, btnText, player1, player2
...
def tradiDrawOX(row, col):
    ...
    tradiCurrPlayer= 'o'
    screen.blit(xImg, (codX, codY))
    turn=False
     
#if a row, column, or diagonal has the same sign, the winning line will be drawn and sent to the other user
def tradiCheckWin():
    global turn, playing, imgWinHon
    win = False
    
    row = 0
    if (gridTradi[row][0] == gridTradi[row][1] == gridTradi[row][2] and gridTradi[row][0] is not None):
    ...
    col = 0
    if gridTradi[0][col] == gridTradi[1][col] == gridTradi[2][col] and gridTradi[0][col] is not None:
    ...
    if gridTradi[0][0] == gridTradi[1][1] == gridTradi[2][2] and gridTradi[0][0] is not None:
    ...
    if gridTradi[0][2] == gridTradi[1][1] == gridTradi[2][0] and gridTradi[0][2] is not None:
...
def tradiGame():
    global gridTradi, score1, score2, row, col, tradiCurrPlayer, turn, playing
    if turn and playing:
        x, y = pygame.mouse.get_pos()
